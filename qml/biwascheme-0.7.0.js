...
if( typeof(BiwaScheme)!='object' ) BiwaScheme={}; with(BiwaScheme) {
  ...
  // load from network
  //
  define_libfunc("load", 1, 2, function(ar, intp){
    var path = ar[0];
    assert_string(path);
    var intp2 = new Interpreter(intp);
		var xhr   = new XMLHttpRequest();
    xhr.open("GET", path, !!ar[1]);

		if(ar[1]) {
			xhr.onreadystatechange = function() {
				if(xhr.status     == 200                 &&
		       xhr.readyState == XMLHttpRequest.DONE) {
					intp2.invoke_closure(ar[1],
															 [intp2.evaluate(xhr.responseText,
																							 function() {
																								 return BiwaScheme.undef;
															                 })]);
				}
			...
			return new BiwaScheme.Pause(function(pause){
				...
				if(xhr.status === 200) {
  				return intp2.evaluate(xhr.responseText,
  															function() {
																	return pause.resume(BiwaScheme.undef);
                                });
				} else {
					console.log("Couldn't load Scheme file.");
				}
			});
    }
  });
  ...
  var _require = function(src, check, proc){
